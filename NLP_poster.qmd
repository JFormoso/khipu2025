...
# 
# # Anotar el texto
# anotaciones <- udpipe_annotate(lematizador, x = tok_all$word)
# anotaciones_df <- as.data.frame(anotaciones)
# 
# anotaciones_df |> 
#   filter(is.na(lemma)) |> 
#   select(token, lemma)
...
#   mutate(token = quitar_tildes(token)) |> 
#   mutate(lemma = case_when(is.na(lemma) & str_detect(token, "rlos$|rlas$|rlo$|rla$|rle$|rse$|rles$|rnos$|rme$") ~ str_remove(token, "los$|las$|lo$|la$|le$|se$|les$|nos$|me$"),
#                            is.na(lemma) & str_detect(token, "andome$") ~ str_replace(token, pattern = "andome$", replacement = "ar"),
#                            is.na(lemma) & str_detect(token, "endome$") ~ str_replace(token, pattern = "endome$", replacement = "r"),
#                            is.na(lemma) & str_detect(token, "andose$") ~ str_replace(token, pattern = "andose$", replacement = "ar"),
#                            is.na(lemma) & str_detect(token, "andola$") ~ str_replace(token, pattern = "andose$", replacement = "ar"),
#                            is.na(lemma) & str_detect(token, "poniendose") ~ "poner",
#                            is.na(lemma) & str_detect(token, "prrfeccionarme") ~ "perfeccionar",
#                            is.na(lemma) & str_detect(token, "desarrolar") ~ "desarrollar",
#                            TRUE ~ lemma)) |> 
#   select(word = token, lemma)
# 
# 
# tok_all <- tok_all |> 
#   mutate(word = quitar_tildes(word)) |> 
#   left_join(anotaciones_df, by = "word")
# 
...
```{r}
tok_c_all_lem <- tok_c_all_lem %>% 
  arrange(desc(n))

total_words_lem <- tok_c_all_lem %>% 
  group_by(document) %>% 
  summarize(total = sum(n))

tok_c_all_lem <- left_join(tok_c_all_lem, total_words_lem)
...
freq_by_rank_lem <- tok_c_all_lem %>% 
  group_by(document) %>% 
  mutate(rank = row_number(), 
         term_frequency = n/total) %>%
  ungroup()
...
bigrams_all <- texto_libre %>% 
  mutate_all(str_to_lower) |>
  mutate_all(quitar_tildes) |> 
  mutate_all(str_remove_all, "ciencia abierta") |> 
  mutate(barreras_ES = str_remove_all(barreras_ES, "barrera|barreras"),
...
sentimientos_binario <- ggplot(emocion_con_lem[9:10,],
                        aes(x = sentimiento,
                            y = total, fill = sentimiento)) + 
  geom_bar(stat = "identity") +
...
```{r}
lda_public_lem <- LDA(dtm_public_lem, k = 2, control = list(seed = 1234))
...
```{r}
lda_interes_stem <- LDA(dtm_interes_stem, k = 2, control = list(seed = 1234))

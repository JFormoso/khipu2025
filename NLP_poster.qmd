...
datos$interes_ES <- quitar_tildes(datos$interes_ES)
datos$publico_general_ES <- quitar_tildes(datos$publico_general_ES)
datos$barreras_ES <- quitar_tildes(datos$barreras_ES)
```
...
tok_all %>% glimpse()
```
### Eliminar Ciencia Abierta de la tokenización

```{r}

tok_barreras_sin_CA <- tok_barreras |> 
  filter(!word %in% c("ciencia", "abierta"))

tok_interes_sin_CA <- tok_interes |> 
  filter(!word %in% c("ciencia", "abierta"))

tok_public_sin_CA <- tok_public |> 
  filter(!word %in% c("ciencia", "abierta"))

tok_all_sin_CA <- tok_all |> 
  filter(!word %in% c("ciencia", "abierta"))

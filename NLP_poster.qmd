...
## Obtener el lema o forma canónica de la palabra
...
# Anotar el texto
anotaciones <- udpipe_annotate(modelo, x = tok_all$word)
...
sentimientos_df <- get_nrc_sentiment(barreras.df, lang="spanish")
sentimientos_df |> glimpse()
```
...
```{r}
# La función get_nrc_sentiments() aplicada solo a barreras
sentimientos_df <- get_nrc_sentiment(palabra.publico.df, lang="spanish")
sentimientos_df |> glimpse()
```

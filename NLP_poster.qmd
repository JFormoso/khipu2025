...
sentimientos_interes <- ggplot(emocion.interes.df[1:8,],
                        aes(x = total,
                            y = sentimiento, fill = sentimiento)) + 
  geom_bar(stat = "identity") +
  labs(title = "Interest",
       x = " ", y = " ")+
  # geom_text(aes(label = total),
...
sentimientos_interes
ggsave("interes_sin_stem.png", width = 5, height = 2, dpi = 300)
```
...
```{r}
sentimientos_interes_meta <- ggplot(emocion.interes.df[1:8,],
                        aes(x = sentimiento,
                            y = total, fill = sentimiento)) + 
...
sentimientos_interes_meta 

ggsave("interes.png", width = 5, height = 1.7, dpi = 300)
```
...
sentimientos_barrera
ggsave("barreras_sin_stem.png", width = 5, height = 2, dpi = 300)
```
...
sentimientos_publico
ggsave("publico_sin_stem.png", width = 5, height = 2, dpi = 300)
```
